...
lda = Artwork.find_by(title: 'Les Demoiselles dAvignon').id
sn =Artwork.find_by(title: 'Starry Night').id
ml = Artwork.find_by(title: 'Mona Lisa').id
vng =Artwork.find_by(title: 'Violin and Grapes').id
...
# artworkshares = ArtworkShare.new({artwork_id: lda, viewer_id: moviebuff} ).save!
